...
class Scraper:
    ...
    def scrape_page_threads(self, page_number):
        ...
        products = soup.find_all("div", class_="product-item-v5")
        with ThreadPoolExecutor(max_workers=6) as executor:
            page_results = list(
                executor.map(
                    scraper.scrape_page,
                    soup.find_all("div", class_="product-item-v5"),
                )
            )
    ...
    def scrape_page(self, product):
        ...
        return {
            ...
            "All Names": product_info.get("all_names", []),  # Store list of names
            "Price": price,
            "URL": product_url,
        }
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=7) as executor:
        results = executor.map(scraper.scrape_page_threads, range(1, total_pages + 1))
