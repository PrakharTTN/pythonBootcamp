...
class EmailEnvironment:
    ...
    '''This function checks for attachments in the mail and Returns a list with names of emails and total number of attachments'''
    def get_attachments(self, msg):
        ...
        if len(attachments) == 0:
            return None, 0, has_image        
        return attachments, len(attachments), has_image
    ...
    '''THis function fetches all emails from inbox'''
    def fetch_emails(self):
        ...
        for email_id in email_ids:
            # Fetch each email
            data = self.session.fetch(email_id, "(RFC822)")
            for response in data:
                if isinstance(response) is tuple:
                    msg = email.message_from_bytes(response[1]) #Response[0]-header file,[1]-mainbody
                    subject = decode_header(msg["subject"])[0][0] #Decodes the subject header
                    date = msg["date"]
                    sender = msg["from"]
                    body_length,body_lines = self.get_body(msg)
                    attachment, total_attachments, has_image = self.get_attachments(msg)
                    ...
                    #In this flag is 3, which mails with images, in the has_image variable
                    #has been passed from attachment functions, which is true if the mail has image
                    #Providing if has_image is false, it will not append
                    elif self.flag == 3 and  not has_image:
                        continue
                    self.emails.append({
                        "subject" : subject,
                        "date" : date,
